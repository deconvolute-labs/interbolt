...
- All exceptions come from `errors.py`, under one base, `InterlockError`. Two branches: decision outcomes (`PolicyViolation`, `PolicyEvaluationError`, `ApprovalDenied`) and misuse (`InterlockConfigError(InterlockError, ValueError)`, `InterlockUsageError(InterlockError, RuntimeError)`). The misuse pair multiply-inherits the fitting builtin so `except InterlockError` and `except ValueError`/`except RuntimeError` both catch correctly. Never raise a generic `Exception`, a bare `ValueError`, or a bare `RuntimeError` anywhere in the library; a config mistake raises `InterlockConfigError`, a bad call sequence raises `InterlockUsageError`. Never add a sixth error class without removing the temptation to over-specialize.
