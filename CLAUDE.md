...
- Container traversal (recursion into builtin containers and mappings, depth bounding, reconstruction with its fail-safe) is implemented in exactly two places: `taint/walk.py` (depth-bounded, leaf-oriented, used at ingress and the sink: `walk_leaves`/`map_leaves`/`leaf_text`) and `taint/wire_walk.py` (path-keyed, used by the serialization contract: `strip_to_json`/`resolve_path`/`replace_at_path`). Never write another recursive container walk. `taint/wire.py` and `taint/wire_rebuild.py` build and rebuild the serialization envelope entirely from `wire_walk.py`'s primitives and perform no recursive descent of their own.
...
- Propagation does not survive an ordinary serialization or storage round trip. Data re-entering the process through any channel other than the serialization contract is fresh untrusted ingress and must be re-`taint`ed. Provenance crosses the boundary only through an explicit `pack`/`unpack` pair; never reconnect re-entered data to a prior label by inference, content matching, or any implicit channel.
...
- All async methods are `async def`. The decision core is pure and synchronous; only the reporter and approval resolver are colored. `asyncio.run()` is never called on a thread the library did not create. Never block the event loop.
