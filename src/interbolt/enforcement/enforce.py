...
def enforce_decision(
    ...
    """Enforce `decision` at a synchronous call site.
...
async def enforce_decision_async(
    decision: Decision, *, approval_resolver: ApprovalResolver
) -> None:
    """Enforce `decision` at an asynchronous call site.

    Same contract as `enforce_decision`, except an awaitable result from
    `approval_resolver` is awaited rather than rejected.
